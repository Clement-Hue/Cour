...
if ( (aLastRow - aFirstRow) * (aLastCol - aFirstCol) +
     (bLastRow - bFirstRow) * (bLastCol - bFirstCol) +
     (aLastRow - aFirstRow) * (bLastCol - bFirstCol) <= CACHE_SIZE * 1024 )
...
stopTime = dtime();
printf("Recursive blocked (%d)\t%d\t%4.6f\n",
       CACHE_SIZE,
       MATRIX_SIZE,
       stopTime - startTime);
